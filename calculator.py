...
from decimal import Decimal, ROUND_HALF_UP   #浮點數
...
class Calculator(QWidget):
    def __init__(self):
        ...
        ''')
    # Check if there is consecutive symbols 檢查符號重複
    def symbdup(self,text):
        if text in ['+', '-', '*', '/']:
            # 檢查輸入的符號是否為連續的運算符號。如果是，則替換最後一個運算符號
    ...
    # Function for calculator button click event 按鈕點擊事件處理
